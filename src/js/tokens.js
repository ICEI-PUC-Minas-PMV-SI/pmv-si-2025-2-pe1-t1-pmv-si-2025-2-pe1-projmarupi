(function () {
  ...
  TokenService.tokensByModule = {
    iaua: {
      introduction: {
        ...
        description:
          "Conquistado por completar a Introdução à Arqueologia Arbórea.",
  ...
  TokenService.addToken = function (tokenId, moduleId, notify = false) {
    ...
    if (
      !Object.keys(TokenService.tokensByModule[moduleId] || {}).includes(
        tokenId
      )
    ) {
      throw new Error(
        `Invalid token ID: ${tokenId} for module ID: ${moduleId}`
      );
    }
