(function () {
  ...
  ProgressService.stepsByModule = {
    iaua: {
      down: {
        ...
        tags: ["Iniciante", "Botânica", "História", "Urbanismo"],
      },
      right: {
        step: 1,
        name: "Progresso",
  ...
  ProgressService.getActualStepByPositon = function (moduleId, stepNumber) {
    ...
    const firstStepId = Object.keys(moduleSteps)
    ...
    if (firstStepId == undefined || moduleSteps[firstStepId] == undefined) {
      throw new Error(
        `Step number:'${stepNumber}' is not valid for Module ID:'${moduleId}'.`
      );
    }
  ...
  ProgressService.setActualStepByPositon = function (moduleId, stepNumber) {
    ...
    const firstStepId = Object.keys(ProgressService.stepsByModule[moduleId])
      .filter((key) => key !== "totalSteps")
      .filter(
        (stepId) =>
          ProgressService.stepsByModule[moduleId][stepId].step === stepNumber
      )
      .at(0);
  ...
  ProgressService.setActualStep = function (moduleId, stepId) {
    ...
    if (ProgressService.stepsByModule[moduleId][stepId] == undefined) {
      throw new Error(
        `Step ID:'${stepId}' is not valid for Module ID:'${moduleId}'.`
      );
    }
    ...
    if (actualStep != undefined) {
      ...
      if (
        ProgressService.stepsByModule[moduleId][stepId].step < actualStep.step
      ) {
        return;
      }
    }
  ...
  ProgressService.getSteps = function (moduleId) {
    ...
    return Object.keys(moduleSteps)
      .filter((key) => key !== "totalSteps")
      .map((key) => {
        return {
          id: key,
